...
def render_related(url):
    """The current page's group siblings, capped — a topical cluster.

    A footer that repeats all fourteen destinations on all fourteen pages is
    boilerplate: the rail and the sheet already carry every one of them as a
    real <a href>. What a tool page can still usefully end with is the handful
    of tools that answer a question like its own, which is exactly the grouping
    nav_data already declares. On a page that is not in the list at all (an
    article, privacy, terms) it falls back to the first N tier-1 tools, so the
    block is never empty.
    """
    cfg = getattr(D, "RELATED", None)
    if not cfg:
        return ""
    limit = cfg.get("limit", 4)
    tier1 = [t for t in D.TOOLS if t["tier"] == 1]
    here = next((t for t in tier1 if canon(t["href"]) == url), None)
    if here:
        pool = [t for t in tier1 if t["group"] == here["group"] and t is not here]
        pool += [t for t in tier1 if t["group"] != here["group"]]
    else:
        pool = tier1
    picks = pool[:limit]
    if not picks:
        return ""
    out = ['<nav class="%s" aria-label="%s">' % (cfg["class"], esc(cfg["aria"])),
           '  <span class="%s-label">%s</span>' % (cfg["class"], esc(cfg["label"]))]
    for t in picks:
        out.append("  " + anchor(t["href"], t["long"], url))
    out.append("</nav>")
    return "\n".join(out)
...
def migrate(text):
    for op in getattr(D, "MIGRATE", []):
        kind = op["op"]
        if kind == "strip":
            # `with` defaults to "" — a plain deletion. Given, it makes this a
            # one-time substitution, which is how a site normalises the stray
            # differences between hand-copied files in the same sweep.
            text = re.sub(op["pattern"], op.get("with", ""), text, count=1, flags=re.S)
            continue
        name = op["region"]
